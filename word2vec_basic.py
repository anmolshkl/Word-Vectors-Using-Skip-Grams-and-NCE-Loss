...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  context_word_index = skip_window + data_index
  b = []
  l = []
  count = 0
  while (count < batch_size):
    window_elements = data[context_word_index - skip_window : context_word_index]
    window_elements.extend(data[context_word_index + 1 : context_word_index + skip_window + 1])
    b.extend([data[context_word_index]] * (len(window_elements)))
    l.extend(window_elements)
    count += window_size - 1
    context_word_index += 1
    data_index += 1

  return np.asarray(b), np.asarray(l).reshape((batch_size, 1))
...
if __name__ == '__main__':
  ...
  with tf.Session(graph=graph) as sess:
    ...
    # You must strat with the pretrained model. 
